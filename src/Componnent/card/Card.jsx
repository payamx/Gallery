...
const Card = ({data, page, fetch, isLoading, error, topics,query}) => {
...
//هاور کردن روی عکس و نشون دادن پروفایل
    ...
    const dispatchFunction = () => {dispatch(fetch())};
    const onHover = (index) => {setHoverIndex(index)};
                    ...
                    <div key={itemIndex}>
                        <div key={itemIndex} className="w-full my-3 px-2 break-inside-avoid  relative  "
                                 ...
                                 onMouseOut={onHoverOver}>
                                    <img className=" rounded-md	hover:brightness-75" src={item?.urls?.small}/>
